...
This is a PyInstaller friendly version of InternetWallpaper script and with notifications, now behaving like a full-fledged
software.
...
def notify(message):
	t = win10toast.ToastNotifier() #Initialize Toast Notifier in variable 't'
	t.show_toast('Presence Wallpaper',message,'PresenceWallpaper.ico',duration=10) #With Custom Icon
...
#Driver Code
def main():
	message = "PresenceWallpaper is fetching your wallpaper now!"
	notify(message)
	sleep(25) #Sleeping for at least 30s to allow for internet connection to be stable. Experiment with these values
	addStartup()
	if check_conn() != True:
		...
		try:
			downloader()
			setwall()
			message = "Here's your wallpaper for the day!"
			notify(message)
		except:
			#Printing a generic error message when I don't know what's wrong... :(
			message = 'Opps! Something went wrong! Make sure you are connected to the internet!'
			notify(message)
